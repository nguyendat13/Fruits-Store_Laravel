        ...
        <section class="my-8">
            <div class="container mx-auto flex flex-col md:flex-row items-start px-4">
                ...
                <div class="w-full md:w-1/2 bg-white p-6 rounded-lg shadow-lg">
                    <img src="{{ asset('images/product/' . $product->thumbnail) }}" alt="{{ $product->name }}" class="rounded-lg object-cover">
                </div>
                <!-- Product Details -->
                <div class="w-full md:w-1/2 md:ml-8 mt-6 md:mt-0">
                    ...
                    <p class="text-gray-700 mb-6">Xuất xứ: {{ $product->brand->name }}</p>
                    <p class="text-gray-700 mb-6">Danh mục: {{ $product->category->name }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    @foreach ($relatedProducts as $relatedProduct)
                        <div class="bg-white p-6 rounded-lg shadow-md hover:shadow-xl transition duration-300 text-center">
                            ...
                            <a href="{{ route('frontend.products.product-detail', $relatedProduct->slug) }}" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition duration-300">Xem chi tiết</a>
                        </div>
                    @endforeach
                </div>
            </div>
