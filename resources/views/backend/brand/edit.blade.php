<x-layout-admin>
    ...
    <div class="flex">
        ...
        <div class="flex-1 p-6 relative left-[100px] w-[1000px]">
            <!-- Header -->
            <div class="mb-6">
                <h1 class="text-3xl font-bold">Chỉnh sửa Brand</h1>
            ...
            <div class="bg-white shadow-md rounded-md p-6">
                <form action="{{ route('brand.update', ['brand' => $brand->id]) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $brand->slug ?? '') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder="Nhập slug" required>
                        @if ($errors->has('slug'))
                    ...
                    <div class="mb-4">
                        ...
                        @if (isset($brand) && $brand->image)
                            <img src="{{ asset('storage/images/brand/' . $brand->image) }}" alt="{{ $brand->image }}" class="mt-2" width="200">
                        @endif
                        @if ($errors->has('image'))
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder="Nhập mô tả">{{ old('description', $brand->description ?? '') }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $brand->sort_order ?? '') }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    ...
                    <!-- Submit -->
                    <div class="mt-6">
                        <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600">Cập nhật Brand</button>
